    ...
    public class BoxBallsController : Controller
    {
        ...
        public IActionResult GetBoxBalls(int ballscount = -1, int concentration = -1, int blackballs = -1, int whiteballs = -1)
        {
            ...
            if ((ballscount < 1 && ballscount != -1) || ballscount > maxBallsCount)
                return Json(new Error(2000, $"Указанное количество шаров в коробке ('ballscount' = {ballscount} шт.) должно находится в границах от 1 до {maxBallsCount} или быть равно '-1' (для случайного значения) исходя из серверных ограничений."));

            //Если концентарция меньше -1 и больше 100, сообщаем об ошибке
            if (concentration < -1 || concentration > 100)
                return Json(new Error(2001, $"Соотношение чёрных шаров к белым ('concentration' = {concentration} %) должно находится в границах от 0 до 100% или быть равно '-1' (для случайного значения)."));
            ...
            if (blackballs < -1 || blackballs > maxBallsCount)
                return Json(new Error(2002, $"Указанное количество чёрных ('blackballs' = {blackballs} шт.) шаров в коробке должно находится в границах от 0 до {maxBallsCount} или быть равно '-1' (для случайного значения) исходя из серверных ограничений."));
            ...
            if (whiteballs < -1 || whiteballs > maxBallsCount)
                return Json(new Error(2003, $"Указанное количество белых ('whiteballs' = {whiteballs} шт.) шаров в коробке должно находится в границах от 0 до {maxBallsCount} или быть равно '-1' (для случайного значения) исходя из серверных ограничений."));
            ...
            if (blackballs > ballscount && ballscount > 0)
                return Json(new Error(2004, $"Указанное количество чёрных шаров ('blackballs' = {blackballs} шт.) не может превышать общее ('ballscount' = {ballscount} шт.) количество шаров в коробке."));

            //Если указанное количество белых шаров превышает общее количество, сообщаем об ошибке
            if (whiteballs > ballscount && ballscount > 0)
                return Json(new Error(2005, $"Указанное количество белых шаров ('whiteballs' = {whiteballs} шт.) не может превышать общее ('ballscount' = {ballscount} шт.) количество шаров в коробке."));
            ...
            if (concentration == 0 && blackballs > 0)
                return Json(new Error(2006, $"При процентном соотношении чёрных шаров к белым ('concentration' = {concentration} %), в корзине невозможно нахождение чёрных шаров."));
            ...
            if (concentration == 100 && whiteballs > 0)
                return Json(new Error(2007, $"При процентном соотношении чёрных шаров к белым ('concentration' = {concentration} %), в корзине невозможно нахождение белых шаров."));
            ...
            if (blackballs + whiteballs == 0 && (blackballs != -1 || whiteballs != -1))
                return Json(new Error(2008, $"Сумма чёрных ('blackballs' = {blackballs} шт.) и белых ('whiteballs' = {whiteballs} шт.) шаров в коробке не должна быть равна 0."));
            ...
            if (blackballs + whiteballs != ballscount && blackballs != -1 && whiteballs != -1 && ballscount != -1)
                return Json(new Error(2009, $"Сумма чёрных ('blackballs' = {blackballs} шт.) и белых ('whiteballs' = {whiteballs} шт.) шаров должна быть равна общему ('ballscount' = {ballscount} шт.) количеству шаров в коробке."));
            ...
            if (blackballs + whiteballs > maxBallsCount && blackballs != -1 && whiteballs != -1)
                return Json(new Error(2010, $"Сумма чёрных ('blackballs' = {blackballs} шт.) и белых ('whiteballs' = {whiteballs} шт.) шаров в коробке не должна быть больше {maxBallsCount} исходя из серверных ограничений."));
            ...
            if (ballscount > 0 && concentration >= 0 && blackballs >= 0)
                if ((int)Math.Round((double)blackballs * (100 - concentration) / concentration + blackballs) != ballscount)
                    return Json(new Error(2011, $"Указанное количество всех шаров ('ballscount' = {ballscount}) не соответствует соотношению концентрации ('concentration' = {concentration}) и количеству чёрных шаров ('blackballs' = {blackballs})."));
            ...
            if (ballscount > 0 && concentration >= 0 && whiteballs >= 0)
                if ((int)Math.Round((double)whiteballs / (100 - concentration) * concentration + whiteballs) != ballscount)
                    return Json(new Error(2012, $"Указанное количество всех шаров ('ballscount' = {ballscount}) не соответствует соотношению концентрации ('concentration' = {concentration}) и количеству белых шаров ('whiteballs' = {whiteballs})."));
            ...
            if (concentration >= 0 && blackballs >= 0 && whiteballs >= 0)
                if (100 - (int)Math.Round((double)whiteballs * concentration / blackballs) != concentration)
                    return Json(new Error(2013, $"Указанная концентрация ('concentration' = {concentration}) не соответствует реальной концентрации при количестве чёрных ('blackballs' = {blackballs}) и белых ('whiteballs' = {whiteballs}) шаров."));
            ...
            if (concentration > 0 && blackballs > 0)
                if ((int)Math.Round((double)(100 - concentration) * blackballs / concentration) + blackballs > maxBallsCount)
                    return Json(new Error(2014, $"Количество всех шаров ('ballscount' = {(int)Math.Round((double)(100 - concentration) * blackballs / concentration) + blackballs}), полученное при указанном количестве чёрных шаров ('blackballs' = {blackballs}) и концентрации ('concentration' = {concentration}) превышает максимально допустимое количество ('maxBallsCount' = {maxBallsCount})."));
            ...
            if (concentration > 0 && whiteballs > 0)
                if ((int)Math.Round((double)concentration / (100 - concentration) * whiteballs) + whiteballs > maxBallsCount)
                    return Json(new Error(2015, $"Количество всех шаров ('ballscount' = {(int)Math.Round((double)concentration / (100 - concentration) * whiteballs) + whiteballs}), полученное при указанном количестве белых шаров ('whiteballs' = {whiteballs}) и концентрации ('concentration' = {concentration}) превышает максимально допустимое количество ('maxBallsCount' = {maxBallsCount})."));
